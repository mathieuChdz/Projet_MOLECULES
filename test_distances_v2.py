...
def run_test():
    ...
    MOL_DIR = r"C:\Users\chedo\Documents\_M2\MOLECULES\Projet_MOLECULES\data\molecules"
    mol_files = [f for f in os.listdir(MOL_DIR) if f.endswith(".mol")]

    ref_mol = os.path.join(MOL_DIR, mol_files[9])
    target_mols = [os.path.join(MOL_DIR, f) for f in mol_files if f != mol_files[9]]

    print(f"--- Molécule de référence : {mol_files[9]} ---")
